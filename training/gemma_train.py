...
class GemmaLoRAClassifier:
    def __init__(self, model_name="google/gemma-2b", num_labels=3, 
        ...
        # Gemma doesn't have a pad token by default, so set it to the eos token
    ...
    def train(self, train_dataset, val_dataset, test_dataset, device, epochs=3, batch_size=8, 
        ...
        """
        Train the Gemma model with LoRA
        """
...
def main():
    parser = argparse.ArgumentParser(description="Finetune Gemma model for ProofWriter classification using LoRA.")
    parser.add_argument('--model_name', type=str, default="google/gemma-2b", help="Gemma model to use (google/gemma-2b, google/gemma-7b)")
    ...
    classifier = GemmaLoRAClassifier(
